...
/*   Updated: 2023/03/08 21:18:50 by aoueldma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	texture_init(t_map *game)
{
	...
	while(i < 4)
	{
		game->create_pixel[i].image_data = (unsigned int *)mlx_get_data_addr(game->create_pixel[i].image, &game->create_pixel[i].bits_per_pixel, &game->create_pixel[i].size_line, &game->create_pixel[i].endian);
		i++;
	}
}